...
function ProductsCard({ products }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3 gap-5 sm:gap-6 w-[100%]">
        ...
        return (
          ...
          >
            ...
            >
              {images.map((img, index) => (
                <SwiperSlide key={index}>
                  <img
                    ...
                    className="w-full h-full object-cover hover:scale-110 transition duration-250"
                  />
                </SwiperSlide>
              ))}
            </Swiper>
              ...
              {featured && (
                <div className="absolute top-3 left-3 sm:top-4 sm:left-4 z-3 flex items-center gap-1.5 bg-accent text-white font-bold text-xs sm:text-sm rounded-xl px-3 py-1.5 sm:px-4 sm:py-2">
                  ...
                  <Star size={16} fill="currentColor" className="hidden sm:block" />
                  Featured
                </div>
              )}
            ...
            <div className="p-4 sm:p-6 flex flex-col flex-1">
              <h3 className="text-xl sm:text-xl font-extrabold text-text-primary">
                {name}
              </h3>

              <p className="text-text-muted text-xs sm:text-sm font-semibold uppercase tracking-wide mt-1">
                {breadcrumb.join(" - ")}
              </p>

              <p className="text-text-secondary text-sm mt-3">{shortDescription}</p>
