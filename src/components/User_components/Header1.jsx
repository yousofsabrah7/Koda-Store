...
function  Header1({modal,Setmodal}){
    ...
    return (
      <div className='bg-surface-card relative top-6 px-6 mt-20 flex justify-between p-3 m-12 rounded-2xl'>
        <div >
        <h1 className='tracking-widest text-accent font-thin'>User Management</h1>
...
<input type="search" placeholder='Search users...'
  className="outline-none text-text-primary [&::-webkit-search-cancel-button]:appearance-none focus:border-[#e2976f]" />
        ...
        <button className='cursor-pointer border border-transparent rounded-4xl p-2.5 hover:border-[#e2976f]' onClick={()=>Setmodal(true)}><FontAwesomeIcon icon={faUserPlus}  /> Add User</button>
        </div>
      </div>
    )
  }
