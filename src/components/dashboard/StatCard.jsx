...
const StatCard = ({ title, value, subtitle, type }) => {
    ...
    return (
        <div className={`bg-slate-900 border relative overflow-hidden p-8 rounded-3xl flex flex-col justify-between gap-4 ${config.cardBg} ${config.border} hover:scale-104 transition`} >
            <div className='flex justify-between gap-4 w-full '>
                <div className={`absolute left-0 top-0 h-1 w-full ${config.iconBg}`} />
                <div className='flex flex-col gap-2'>
                    <p className='font-subBold text-slate-500'>{title}</p>
            ...
            <div className="mt-8 h-[2px] bg-gradient-to-r from-transparent via-slate-600 to-transparent" />
        </div>
    )
}
...
const cardConfig = {
    ...
    topProduct: {
        icon: Package,
        iconBg: "bg-gradient-to-br  bg-gradient-to-br from-violet-500 to-fuchsia-500",
