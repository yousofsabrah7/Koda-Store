...
function TableCom({ isLoading,tbody,thead,Arrycolumns}) {
...
// Arraycoluns : just array of names colums , and we use here beacuse  exapmle : grid-cold-[Arraycolumns.length]
  return (
    <table className="bg-surface-card min-w-max md:w-full h-auto px-8 ">
      <thead>
        <tr className={`*:text-sm  cursor-pointer  gap-8 *:p-2 min-w-max   md:grid-cols-${Arrycolumns.length}   px-8  bg-surface-card   border-t border-border-subtle *:text-text-muted`}>
         {thead}
        </tr>
      </thead>
      {isLoading ? (<TableSkeleton columns={Arrycolumns.length+1}  />) :
