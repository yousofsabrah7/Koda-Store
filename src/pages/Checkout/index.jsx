...
const inputClass = (invalid) =>
  [
    "w-full rounded-lg border bg-surface-card px-3 py-2 text-sm",
    "text-text-primary placeholder:text-text-muted transition",
    "focus:outline-none focus:ring-[3px] focus:ring-accent-light",
    invalid
...
function Checkout() {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) {
      next.name = "Enter the name for delivery.";
    }
  ...
  if (cartError) {
    return (
      <div className="grid min-h-screen place-items-center bg-surface-base px-4 text-center">
  ...
  return (
    <div className="min-h-screen bg-surface-base font-sans">
      <div className="mx-auto max-w-[1060px] px-4 pb-16 pt-6">
        <h1 className="mb-5 text-[21px] font-bold tracking-tight text-text-primary">
          Checkout
        </h1>
              ...
              {items.map((item) => (
                ...
                >
                  {item.image ? (
                    ...
                    <div className="h-10 w-10 flex-none rounded-md bg-surface-elevated" />
                  )}
